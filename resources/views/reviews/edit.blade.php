    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('reviews.update', $review->id) }}" method="POST">
                @csrf
                @method('PUT')
                <x-string-field name="pseudo" :value="$review->pseudo" required="1" />
                <x-text-field name="comment" :value="$review->comment" required="1" />
                <x-string-field name="rating" type="number" :value="$review->rating" required="1" />
                <div class="form-group">
                    <label for="approved">Moderation status:</label>
                    <select name="approved" id="approved" class="form-control @error('approved')is-invalid @enderror" required>
